...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, onOpenDemoTour }) => {
  const navItems = [
    ...
    { id: 'ports', icon: Anchor, label: 'Port Radar' },
    { id: 'analytics', icon: BarChart3, label: 'Analytics Dashboard' },
    { id: 'warehouse', icon: Layers, label: 'Warehouse Yard Twin' },
  ];
  ...
  return (
    <aside className="w-16 md:w-20 bg-white border-r border-[#EAEFF4] flex flex-col items-center py-6 justify-between shrink-0 shadow-sm z-30">
      ...
      <div className="flex flex-col items-center gap-6">
        ...
        >
          {/* Circular spiral/container logo inspired by reference */}
      ...
      <div className="flex flex-col items-center gap-3">
        ...
        <button 
          ...
          className="w-11 h-11 rounded-2xl text-[#94A3B8] hover:text-[#0F172A] hover:bg-[#F1F5F9] flex items-center justify-center transition-colors"
        >
          <Settings size={20} />
        </button>
        <button 
